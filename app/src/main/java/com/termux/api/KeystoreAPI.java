 ...
 * <ul>
 *     <li>detailed: if set, key parameters (modulus etc.) are included in the
 * response</li>
 * </ul>
 */
@RequiresApi(api = Build.VERSION_CODES.M)
private static void listKeys(TermuxApiReceiver apiReceiver,
                             final Intent intent) {
 ...
 */
@RequiresApi(api = Build.VERSION_CODES.M)
private static void printPrivateKey(JsonWriter out, PrivateKeyEntry entry,
                                    boolean detailed)
throws GeneralSecurityException, IOException {
	PrivateKey privateKey = entry.getPrivateKey();
	String algorithm = privateKey.getAlgorithm();
	KeyInfo keyInfo =
		KeyFactory.getInstance(algorithm).getKeySpec(privateKey, KeyInfo.class);
 ...
 * </ul>
 */
private static void deleteKey(TermuxApiReceiver apiReceiver,
                              final Intent intent) {
 ...
 *     <li>alias: key alias</li>
 *     <li>
 *         algorithm: key algorithm, should be one of the
 * KeyProperties.KEY_ALGORITHM_* values, for example {@link
 * KeyProperties#KEY_ALGORITHM_RSA} or
 ...
 *         {@link KeyProperties#PURPOSE_SIGN}+{@link
 * KeyProperties#PURPOSE_VERIFY}
 *     </li>
 *     <li>
 *         digests: set of hashes this key can be used with, should be an
 * array of KeyProperties.DIGEST_* values, for example
 *         {@link KeyProperties#DIGEST_SHA256} and {@link
 * KeyProperties#DIGEST_SHA512}
 *     </li>
 *     <li>size: key size, only used for RSA keys</li>
 *     <li>curve: elliptic curve name, only used for EC keys</li>
 *     <li>
 *         userValidity: number of seconds where it is allowed to use this key
 * for signing after unlocking the device (re-locking and unlocking restarts
 * the timer), if set to 0 this feature is disabled (i.e. the key can be used
 * anytime)
 *     </li>
 * </ul>
 */
...
private static void generateKey(TermuxApiReceiver apiReceiver,
                                final Intent intent) {
	ResultReturner.returnData(apiReceiver, intent, out->{
			String alias = intent.getStringExtra("alias");
...
/**
 * Sign a given byte stream. The file is read from stdin and the signature is
 * output to stdout. The output is encoded using base64.<br> Required intent
 * extras: <ul> <li>alias: key alias</li> <li> algorithm: key algorithm and
 * hash combination to use, e.g. SHA512withRSA (the full list can be found at
 *         <a
 * href="https://developer.android.com/training/articles/keystore#SupportedSignatures">
 *         the Android documentation</a>)
 *     </li>
 * </ul>
 */
private static void signData(TermuxApiReceiver apiReceiver,
                             final Intent intent) {
 ...
 * The file is read from stdin, and a "true" or "false" message is printed to
 * the stdout.<br> Required intent extras: <ul> <li>alias: key alias</li> <li>
 *         algorithm: key algorithm and hash combination that was used to
 * create this signature, e.g. SHA512withRSA (the full list can be found at <a
 * href="https://developer.android.com/training/articles/keystore#SupportedSignatures">
 *         the Android documentation</a>)
 *     </li>
 ...
 * </ul>
 */
private static void verifyData(TermuxApiReceiver apiReceiver,
                               final Intent intent) {
	ResultReturner.returnData(apiReceiver, intent, new WithInput() {
			@Override
			...
			throws GeneralSecurityException, IOException {
			        ...
			        if (signatureFile.length() != read)
					out.println(false);

			        Signature signature = Signature.getInstance(algorithm);
			        signature.initVerify(
					getKeyStore().getCertificate(alias).getPublicKey());
			        signature.update(input);
...
private static void printErrorMessage(TermuxApiReceiver apiReceiver,
                                      Intent intent) {
	ResultReturner.returnData(
		apiReceiver, intent,
		out
		->out.println(
			"termux-keystore requires at least Android 6.0 (Marshmallow)."));
}
}
