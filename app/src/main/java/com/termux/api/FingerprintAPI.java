...
/**
 * This API allows users to use device fingerprint sensor as an authentication
 * mechanism
 */
...
// error constants
protected static final String ERROR_UNSUPPORTED_OS_VERSION =
	"ERROR_UNSUPPORTED_OS_VERSION";
protected static final String ERROR_NO_HARDWARE = "ERROR_NO_HARDWARE";
protected static final String ERROR_NO_ENROLLED_FINGERPRINTS =
...
protected static final String ERROR_TOO_MANY_FAILED_ATTEMPTS =
...
protected static FingerprintResult fingerprintResult =
...
/**
 * Handles setup of fingerprint sensor and writes Fingerprint result to
 * console
 */
static void onReceive(final Context context, final Intent intent) {
	...
	if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
		...
		// make sure we have a valid fingerprint sensor before attempting to
		// launch Fingerprint activity
		if (validateFingerprintSensor(context, fingerprintManagerCompat)) {
			Intent fingerprintIntent =
				new Intent(context, FingerprintActivity.class);
 ...
 */
protected static void postFingerprintResult(Context context, Intent intent,
                                            final FingerprintResult result) {
	ResultReturner.returnData(
		context, intent, new ResultReturner.ResultJsonWriter() {
			@Override
...
/**
 * Ensure that we have a fingerprint sensor and that the user has already
 * enrolled fingerprints
 */
@TargetApi(Build.VERSION_CODES.M)
protected static boolean
validateFingerprintSensor(Context context,
                          FingerprintManagerCompat fingerprintManagerCompat) {
	boolean result = true;

	if (!fingerprintManagerCompat.isHardwareDetected()) {
		Toast
		.makeText(context, "No fingerprint scanner found!",
		          Toast.LENGTH_SHORT)
		.show();
	...
	if (!fingerprintManagerCompat.hasEnrolledFingerprints()) {
		Toast.makeText(context, "No fingerprints enrolled", Toast.LENGTH_SHORT)
		.show();
...
authenticateWithFingerprint(final FragmentActivity context,
                            final Intent intent, final Executor executor) {
	BiometricPrompt biometricPrompt = new BiometricPrompt(
		context, executor, new BiometricPrompt.AuthenticationCallback() {
				@Override
				public void onAuthenticationError(int errorCode,
				        ...
				        if (errorCode == BiometricConstants.ERROR_LOCKOUT) {
				                ...
				                // first time locked out, subsequent auth attempts will fail
				                // immediately for a bit
	...
	builder.setTitle(intent.hasExtra("title") ? intent.getStringExtra("title")
	                                        : "Authenticate");
...
/**
 * Adds a timeout for our fingerprint sensor which will force a result
 * return if we haven't already received one
 */
protected static void
addSensorTimeout(final Context context, final Intent intent,
...
/**
 * Simple class to encapsulate information about result of a fingerprint
