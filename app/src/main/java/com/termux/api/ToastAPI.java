...
public class ToastAPI {
  ...
  public static void onReceive(final Context context, Intent intent) {
    ...
    ResultReturner.returnData(
        context, intent, new ResultReturner.WithStringInput() {
          @Override
  ...
  protected static int getColorExtra(Intent intent, String extra,
                                     int defaultColor) {
    ...
    if (intent.hasExtra(extra)) {
      ...
      try {
        ...
        TermuxApiLogger.error(String.format(
            "Failed to parse color '%s' for '%s'", colorExtra, extra));
      }
    }
    return color;
  }
