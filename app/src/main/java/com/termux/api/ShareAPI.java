...
static void onReceive(TermuxApiReceiver apiReceiver, final Context context,
	...
	final boolean defaultReceiverExtra =
		intent.getBooleanExtra("default-receiver", false);
	...
	if (actionExtra == null) {
		...
		case "view":
			...
			break;
		default:
			TermuxApiLogger.error("Invalid action '" + actionExtra +
			                      "', using 'view'");
			break;
		}
	}
	...
	if (fileExtra == null) {
		...
		ResultReturner.returnData(
			apiReceiver, intent, new ResultReturner.WithStringInput() {
				...
				public void writeResult(PrintWriter out) {
				        ...
				        if (titleExtra != null)
						sendIntent.putExtra(Intent.EXTRA_SUBJECT, titleExtra);
				        sendIntent.setType(contentTypeExtra == null ? "text/plain"
				                          : contentTypeExtra);
						...
						.createChooser(sendIntent,
								       ...
								       R.string.share_file_chooser_title))
		...
		ResultReturner.returnData(apiReceiver, intent, out->{
				...
				if (!(fileToShare.isFile() && fileToShare.canRead())) {
				        out.println("ERROR: Not a readable file: '" +
				...
				Intent sendIntent = new Intent();
				sendIntent.setAction(finalIntentAction);
				Uri uriToShare = Uri.parse("content://com.termux.sharedfiles" +
				                           fileToShare.getAbsolutePath());
				sendIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK |
				...
				if (contentTypeExtra == null) {
				        ...
				        contentTypeToUse =
						mimeTypes.getMimeTypeFromExtension(fileExtension.toLowerCase());
