...
public abstract class ResultReturner {
  ...
  /**
   * An extra intent parameter which specifies a linux abstract namespace socket
   * address where output from the API call should be written.
   */
  private static final String SOCKET_OUTPUT_EXTRA = "socket_output";

  /**
   * An extra intent parameter which specifies a linux abstract namespace socket
   * address where input to the API call can be read from.
   */
  ...
  /**
   * Possible subclass of {@link ResultWriter} when input is to be read from
   * stdin.
   */
  ...
  /**
   * Possible marker interface for a {@link ResultWriter} when input is to be
   * read from stdin.
   */
  ...
  public static abstract class WithAncillaryFd implements ResultWriter {
    ...
    public final void setFd(int newFd) { fd = newFd; }

    public final int getFd() { return fd; }
  }
  ...
  public static void copyIntentExtras(Intent origIntent, Intent newIntent) {
    ...
    newIntent.putExtra(SOCKET_OUTPUT_EXTRA,
                       origIntent.getStringExtra(SOCKET_OUTPUT_EXTRA));
    newIntent.putExtra(SOCKET_INPUT_EXTRA,
                       origIntent.getStringExtra(SOCKET_INPUT_EXTRA));
  }

  /**
   * Run in a separate thread, unless the context is an IntentService.
   */
  public static void returnData(Object context, final Intent intent,
                                final ResultWriter resultWriter) {
    final PendingResult asyncResult =
        (context instanceof BroadcastReceiver)
            ? ((BroadcastReceiver)context).goAsync()
            : null;
    ...
    final Runnable runnable = () -> {
      try {
        ...
        try (LocalSocket outputSocket = new LocalSocket()) {
          String outputSocketAdress =
              intent.getStringExtra(SOCKET_OUTPUT_EXTRA);
            ...
            if (resultWriter != null) {
              if (resultWriter instanceof WithInput) {
                try (LocalSocket inputSocket = new LocalSocket()) {
                  String inputSocketAdress =
                      intent.getStringExtra(SOCKET_INPUT_EXTRA);
                  inputSocket.connect(
                      new LocalSocketAddress(inputSocketAdress));
                  ((WithInput)resultWriter)
                      .setInput(inputSocket.getInputStream());
