...
public void onReceive(Context context, Intent intent) {
	try {
		...
		// Make sure never to throw exception from BroadCastReceiver to avoid
		// "process is bad" behaviour from the Android system.
...
private void doWork(Context context, Intent intent) {
	...
	case "Brightness":
		if ((Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) && (!Settings.System.canWrite(context))) {
			...
			Toast
			.makeText(context, "Please enable permission for Termux:API",
			          Toast.LENGTH_LONG)
			.show();
	...
	case "NotificationList":
		...
		final boolean NotificationServiceEnabled =
			flat != null && flat.contains(cn.flattenToString());
		if (!NotificationServiceEnabled) {
			Toast
			.makeText(context, "Please give Termux:API Notification Access",
			          Toast.LENGTH_LONG)
			.show();
