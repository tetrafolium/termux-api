    ...
    ResultReturner.returnData(
        apiReceiver, intent, new ResultReturner.ResultJsonWriter() {
          public void writeJson(JsonWriter out) throws Exception {
            ...
            out.name("PROPERTY_OUTPUT_FRAMES_PER_BUFFER")
                .value(framesPerBuffer);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
              ...
              if (sr_ll != sr || bs_ll != bs) { // all or nothing
                ...
                out.name("AUDIOTRACK_BUFFER_SIZE_IN_FRAMES_LOW_LATENCY")
                    .value(bs_ll);
              }
              if (sr_ps != sr || bs_ps != bs) { // all or nothing
                out.name("AUDIOTRACK_SAMPLE_RATE_POWER_SAVING").value(sr_ps);
                out.name("AUDIOTRACK_BUFFER_SIZE_IN_FRAMES_POWER_SAVING")
                    .value(bs_ps);
              }
            } else {
