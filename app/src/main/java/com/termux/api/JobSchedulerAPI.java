...
private static String formatJobInfo(JobInfo jobInfo) {
	final String path =
		jobInfo.getExtras().getString(SchedulerJobService.SCRIPT_FILE_PATH);
	List<String> description = new ArrayList<String>();
	if (jobInfo.isPeriodic()) {
		description.add(String.format(Locale.ENGLISH, "(periodic: %dms)",
	...
	if (Build.VERSION.SDK_INT >= 28) {
		description.add(String.format(Locale.ENGLISH, "(network: %s)",
		                              jobInfo.getRequiredNetwork().toString()));
	}

	return String.format(Locale.ENGLISH, "Job %d: %s\t%s", jobInfo.getId(),
	                     path, TextUtils.join(" ", description));
}

static void onReceive(TermuxApiReceiver apiReceiver, Context context,
                      Intent intent) {
	...
	final boolean batteryNotLow =
		intent.getBooleanExtra("battery_not_low", true);
	...
	final boolean storageNotLow =
		intent.getBooleanExtra("storage_not_low", false);
	...
	if (cancelAll) {
		...
		ResultReturner.returnData(apiReceiver, intent,
		                          out->out.println("Cancelling all jobs"));
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
			...
			ResultReturner.returnData(
				...
				out
				->out.println(
					"Need at least Android N to cancel individual jobs"));
		}
		return;
	}
	...
	if (scriptPath == null) {
		ResultReturner.returnData(apiReceiver, intent,
		                          out->out.println("No script path given"));
		return;
	}
	...
	if (!fileCheckMsg.isEmpty()) {
		ResultReturner.returnData(
			apiReceiver, intent,
			out->out.println(String.format(fileCheckMsg, scriptPath)));
		return;
	}

	PersistableBundle extras = new PersistableBundle();
	extras.putString(SchedulerJobService.SCRIPT_FILE_PATH,
	                 file.getAbsolutePath());

	ComponentName serviceComponent =
		new ComponentName(context, SchedulerJobService.class);
	...
	final String message =
		String.format(Locale.ENGLISH, "Scheduling %s - response %d",
		              formatJobInfo(job), scheduleResponse);
...
private static void displayPendingJobs(TermuxApiReceiver apiReceiver,
                                       Intent intent,
                                       JobScheduler jobScheduler) {
	...
	if (jobs.isEmpty()) {
		ResultReturner.returnData(apiReceiver, intent,
		                          out->out.println("No pending jobs"));
		return;
	}
...
private static void cancelJob(TermuxApiReceiver apiReceiver, Intent intent,
                              JobScheduler jobScheduler, int jobId) {
	final JobInfo jobInfo = jobScheduler.getPendingJob(jobId);
	if (jobInfo == null) {
		ResultReturner.returnData(apiReceiver, intent,
		                          out
		                          ->out.println(String.format(
								Locale.ENGLISH, "No job %d found", jobId)));
		return;
	}
	ResultReturner.returnData(
		apiReceiver, intent,
		out
		->out.println(String.format(Locale.ENGLISH, "Cancelling %s",
