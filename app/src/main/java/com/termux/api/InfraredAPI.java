...
static void onReceiveCarrierFrequency(TermuxApiReceiver apiReceiver,
			        ...
			        if (irManager.hasIrEmitter()) {
						...
						irManager.getCarrierFrequencies();
			                if (ranges == null) {
			                        out.beginObject()
			                        .name("API_ERROR")
			                        .value("Error communicating with the Consumer IR Service")
			                        .endObject();
					} else {
			                        out.beginArray();
...
static void onReceiveTransmit(TermuxApiReceiver apiReceiver,
