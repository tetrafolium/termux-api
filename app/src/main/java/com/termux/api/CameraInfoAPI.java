    ...
    ResultReturner.returnData(apiReceiver, intent, new ResultJsonWriter() {
      ...
      public void writeJson(JsonWriter out) throws Exception {
        ...
        for (String cameraId : manager.getCameraIdList()) {
          ...
          int[] flashModeValues =
              camera.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_MODES);
          ...
          SizeF physicalSize =
              camera.get(CameraCharacteristics.SENSOR_INFO_PHYSICAL_SIZE);
          out.name("physical_size")
              .beginObject()
              .name("width")
              .value(physicalSize.getWidth())
              .name("height")
              .value(physicalSize.getHeight())
              .endObject();

          out.name("capabilities").beginArray();
          for (int capability : camera.get(
