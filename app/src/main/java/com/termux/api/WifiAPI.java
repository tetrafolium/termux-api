...
static void onReceiveWifiConnectionInfo(TermuxApiReceiver apiReceiver,
		...
		apiReceiver, intent, new ResultReturner.ResultJsonWriter() {
			...
			public void writeJson(JsonWriter out) throws Exception {
			        ...
			        if (info == null) {
			                ...
			                // noinspection deprecation - formatIpAddress is deprecated, but
			                // we only have a ipv4 address here:
			                out.name("ip").value(
						Formatter.formatIpAddress(info.getIpAddress()));
			                ...
			                out.name("supplicant_state")
			                .value(info.getSupplicantState().toString());
				}
			        out.endObject();
			}
		});
...
static void onReceiveWifiScanInfo(TermuxApiReceiver apiReceiver,
                                  final Context context,
                                  final Intent intent) {
	ResultReturner.returnData(
		apiReceiver, intent, new ResultReturner.ResultJsonWriter() {
							...
							case ScanResult.CHANNEL_WIDTH_160MHZ:
								...
								break;
			                                ...
			                                if (channelWidth != ScanResult.CHANNEL_WIDTH_20MHZ) {
			                                        // centerFreq0 says "Not used if the AP bandwidth is 20
			                                        // MHz".
			                                        out.name("center_frequency_mhz").value(scan.centerFreq0);
							}
			                                if (!TextUtils.isEmpty(scan.operatorFriendlyName)) {
			                                        out.name("operator_name")
			                                        .value(scan.operatorFriendlyName.toString());
							}
...
static void onReceiveWifiEnable(TermuxApiReceiver apiReceiver,
