...
private static void toggleTorch(Context context, boolean enabled) {
	try {
		...
		if (torchCameraId != null) {
			...
			Toast
			.makeText(context, "Torch unavailable on your device",
			          Toast.LENGTH_LONG)
			.show();
		}
...
private static String getTorchCameraId(CameraManager cameraManager)
throws CameraAccessException {
	String[] cameraIdList = cameraManager.getCameraIdList();
	String result = null;

	for (String id : cameraIdList) {
		if (cameraManager.getCameraCharacteristics(id).get(
