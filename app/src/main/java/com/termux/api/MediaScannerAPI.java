...
public class MediaScannerAPI {
  ...
  static void onReceive(TermuxApiReceiver apiReceiver, final Context context,
    ...
    ResultReturner.returnData(apiReceiver, intent, out -> {
      ...
      if (recursive)
        scanFilesRecursively(out, context, filePaths, totalScanned, verbose);
      out.println(String.format(Locale.ENGLISH, "Finished scanning %d file(s)",
                                totalScanned[0]));
    });
  }

  private static void scanFiles(PrintWriter out, Context context,
                                String[] filePaths, Integer[] totalScanned,
                                final Boolean verbose) {
    MediaScannerConnection.scanFile(
        context.getApplicationContext(), filePaths, null,
        (path, uri)
            -> TermuxApiLogger.info("'" + path + "'" +
                                    (uri != null ? " -> '" + uri + "'" : "")));
  ...
  private static void scanFilesRecursively(PrintWriter out, Context context,
                                           String[] filePaths,
      ...
      while (currentPath != null && currentPath.isDirectory() &&
        ...
        try {
          ...
          TermuxApiLogger.error(
              String.format("Failed to open '%s'", currentPath.toString()), e);
        }
