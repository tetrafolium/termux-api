...
/**
 * API that allows receiving user input interactively in a variety of different
 * ways
 */
public class DialogActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    String methodType = intent.hasExtra("input_method")
                            ? intent.getStringExtra("input_method")
                            : "";
  ...
  protected void postResult(final Context context, final InputResult result) {
    ResultReturner.returnData(
        context, getIntent(), new ResultReturner.ResultJsonWriter() {
          @Override
  ...
  /**
   * Factory for returning proper input method type that we received in our
   * incoming intent
   */
  static class InputMethodFactory {

    public static InputMethod get(final String type,
  ...
  static class CheckBoxInputMethod extends InputDialog<LinearLayout> {
    ...
    CheckBoxInputMethod(AppCompatActivity activity) { super(activity); }
  ...
  static class ConfirmInputMethod extends InputDialog<TextView> {
    ...
    ConfirmInputMethod(AppCompatActivity activity) { super(activity); }
    ...
    TextView createWidgetView(AppCompatActivity activity) {
      ...
      String text = intent.hasExtra("input_hint")
                        ? intent.getStringExtra("input_hint")
                        : "Confirm";
  ...
  static class CounterInputMethod extends InputDialog<View> {
    ...
    CounterInputMethod(AppCompatActivity activity) { super(activity); }
    ...
    void updateLabel() { counterLabel.setText(String.valueOf(counter)); }
  ...
  static class DateInputMethod extends InputDialog<DatePicker> {
    ...
    DateInputMethod(AppCompatActivity activity) { super(activity); }
  ...
  static class TextInputMethod extends InputDialog<EditText> {
    ...
    TextInputMethod(AppCompatActivity activity) { super(activity); }
    ...
    EditText createWidgetView(AppCompatActivity activity) {
      ...
      if (password) {
        flags = numeric ? (flags | InputType.TYPE_NUMBER_VARIATION_PASSWORD)
  ...
  static class TimeInputMethod extends InputDialog<TimePicker> {
    ...
    TimeInputMethod(AppCompatActivity activity) { super(activity); }
    ...
    String getResult() {
      ...
      if (Build.VERSION.SDK_INT > Build.VERSION_CODES.M) {
        result = String.format(Locale.getDefault(), "%02d:%02d",
                               widgetView.getHour(), widgetView.getMinute());
      } else {
        result = String.format(Locale.getDefault(), "%02d:%02d",
                               widgetView.getCurrentHour(),
                               widgetView.getCurrentMinute());
      }
      return result;
    }
  ...
  static class RadioInputMethod extends InputDialog<RadioGroup> {
    ...
    RadioInputMethod(AppCompatActivity activity) { super(activity); }
    ...
    @Override
    String getResult() {
      int radioIndex = radioGroup.indexOfChild(
          widgetView.findViewById(radioGroup.getCheckedRadioButtonId()));
   ...
   */
  public static class BottomSheetInputMethod
      extends BottomSheetDialogFragment implements InputMethod {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
      ...
      return new BottomSheetDialog(getActivity(), getTheme()) {
        ...
        public void onBackPressed() {
          super.onBackPressed();
          // make it so that user only has to hit back key one time to get rid
          // of bottom sheet
    ...
    public void setupDialog(final Dialog dialog, int style) {
      ...
      final String[] values =
          getInputValues(Objects.requireNonNull(getActivity()).getIntent());
    ...
    /**
     * These keyboard methods exist to work around inconsistent show / hide
     * behavior from canceling BottomSheetDialog and produces the desired result
     * of hiding keyboard on creation of dialog and showing it after a selection
     * or cancellation, as long as we are still within the Termux application
     */
    ...
    protected void showKeyboard() {
      getInputMethodManager().showSoftInput(getView(),
                                            InputMethodManager.SHOW_FORCED);
    }

    protected InputMethodManager getInputMethodManager() {
      return (InputMethodManager)Objects.requireNonNull(getContext())
  ...
  static class SpinnerInputMethod extends InputDialog<Spinner> {
    ...
    SpinnerInputMethod(AppCompatActivity activity) { super(activity); }
    ...
    Spinner createWidgetView(AppCompatActivity activity) {
      ...
      final ArrayAdapter<String> adapter =
          new ArrayAdapter<>(activity, R.layout.spinner_item, items);
  ...
  static class SpeechInputMethod extends InputDialog<TextView> {
    ...
    SpeechInputMethod(AppCompatActivity activity) { super(activity); }
    ...
    @Override
    public void create(final AppCompatActivity activity,
                       final InputResultListener resultListener) {
      // Since we're using the microphone, we need to make sure we have proper
      // permission
      ...
      if (!hasSpeechRecognizer(activity)) {
        Toast
            .makeText(activity, "No voice recognition found!",
                      Toast.LENGTH_SHORT)
            .show();
        activity.finish();
      }
      ...
      final SpeechRecognizer recognizer =
          createSpeechRecognizer(activity, resultListener);

      // create intermediate InputResultListener so that we can stop our speech
      // listening if user hits the cancel button
      DialogInterface.OnClickListener clickListener =
          getClickListener(result -> {
    ...
    createSpeechRecognizer(AppCompatActivity activity,
                           final InputResultListener listener) {
  ...
  /**
   * Base Dialog class to extend from for adding specific views / widgets to a
   * Dialog interface
  ...
  abstract static class InputDialog<T extends View> implements InputMethod {
    ...
    // method to be implemented that handles creating view that is placed in our
    // dialog
    abstract T createWidgetView(AppCompatActivity activity);

    // method that should be implemented that handles returning a result
    // obtained through user input
    ...
    public void create(AppCompatActivity activity,
      ...
      DialogInterface.OnClickListener clickListener =
          getClickListener(resultListener);
    ...
    getClickListener(final InputResultListener listener) {
    ...
    /**
     * Creates a dialog builder to initialize a dialog w/ a view and button
     * click listeners
     */
    AlertDialog.Builder
    getDialogBuilder(AppCompatActivity activity,
                     DialogInterface.OnClickListener clickListener) {
      final Intent intent = activity.getIntent();
      final View layoutView = getLayoutView(activity, widgetView);

      return new AlertDialog.Builder(activity)
          .setTitle(intent.hasExtra("input_title")
    ...
    String getNegativeButtonText() { return "Cancel"; }

    String getPositiveButtonText() { return "OK"; }

    void onDismissed() { postCanceledResult(); }

    /**
     * Create a basic frame layout that will add a margin around our main widget
     * view
     */
    ...
    /**
     * Returns an InputResult containing code of our button and the text if we
     * hit OK
     */
    InputResult onDialogClick(int button) {
